package util
{
	...
	public class Util
	{
		...
		public static function digitsWithHour(num:Number):String
		{
			var hour:int = Math.floor(num / 3600);
			var min:int = Math.floor((num % 3600) / 60);
			var sen:int = Math.floor(num % 60);
			var str:String;
			if (hour == 0)
			{
				str = zeroPad(min) + ":" + zeroPad(sen);
			}
			else
			{
				str = hour + ":" + zeroPad(min) + ":" + zeroPad(sen);
			}
		...
		public static function calculateX(bigWidth:int, samllWidth:int, length:int, index:int):int
		{
			var gap:int = (bigWidth - samllWidth * length) / (length + 1);
			return gap + samllWidth * 0.5 + index * (samllWidth + gap);
		}
		
		public static function leaveOutMoreText(text:TextField, width:int):void
		{
			var shorted:Boolean = false;
			while (text.textWidth > width)
			{
				text.text = text.text.substr(0, text.text.length - 2);
				shorted = true;
			}
			if (shorted)
				text.appendText("...");
		...
		public static function getDomain():String
		{
			...
			if (ExternalInterface.available)
			{
				try
				{
					domain = ExternalInterface.call('eval', 'window.location.host');
					domain = "http://" + domain;
					log(Util, domain);
				}
				catch (ex:*)
		...
		{
			trace(getQualifiedClassName(caller) + "--->", obj);
			Log.log(getQualifiedClassName(caller) + "--->" + obj);
