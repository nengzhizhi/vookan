package cc.hl.model.comment.space
{
   ...
   public class CommentSpaceManager extends Object
   {
      ...
      public function add(param1:Comment) : void {
         ...
         if(param1.height >= this.Height - util.GlobalData.offsetDownHeight)
         {
      ...
      protected function setY(param1:Comment, param2:int = 0) : void {
         ...
         if(_loc4_.length == 0)
         {
            param1.setY(util.GlobalData.offsetUpHeight,param2,this.transformY);
            _loc4_.push(param1);
            return;
         }
         if(this.vCheck(util.GlobalData.offsetUpHeight,param1,param2))
         {
            param1.setY(util.GlobalData.offsetUpHeight,param2,this.transformY);
            Util.binsert(_loc4_,param1,this.bottom_cmp);
            return;
         }
         for each(_loc5_ in _loc4_)
         {
            _loc3_ = _loc5_.bottom + 1;
            if(_loc3_ + param1.height > this.Height - util.GlobalData.offsetDownHeight)
            {
               break;
            }
      ...
      public function updateY(param1:int) : int {
         if(param1 > util.GlobalData.root.stage.stageHeight * 0.35 && param1 < util.GlobalData.root.stage.stageHeight * 0.65)
         {
            if(Math.random() >= 0)
            {
               param1 = util.GlobalData.root.stage.stageHeight * 0.65;
            }
         }
         return param1;
      }
